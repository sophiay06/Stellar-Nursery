...
namespace SpaceGraphicsToolkit.LightAndShadow
{
	...
	public abstract class SgtShadow : MonoBehaviour
	{
		...
		private static Vector4[]   tempRingRatio  = new Vector4[MAX_RING_SHADOWS];
		...
		public static void WriteRing(int maxShadows)
		{
			...
			for (var i = 0; i < tempShadows.Count && shadowCount < maxShadows; i++)
			{
				...
				if (shadowRing != null)
				{
					...
					tempRingRatio[shadowCount] = new Vector4(shadow.Ratio, 0.0f, 0.0f, 0.0f);
			...
			foreach (var tempMaterial in CwHelper.tempMaterials)
			{
				...
				if (shadowCount > 0)
				{
					...
					tempMaterial.SetVectorArray(_SGT_RingShadowRatio, tempRingRatio);
				}
			}
			...
			foreach (var tempProperties in CwHelper.tempProperties)
			{
				...
				if (shadowCount > 0)
				{
					...
					tempProperties.SetVectorArray(_SGT_RingShadowRatio, tempRingRatio);
				}
			}
		}
