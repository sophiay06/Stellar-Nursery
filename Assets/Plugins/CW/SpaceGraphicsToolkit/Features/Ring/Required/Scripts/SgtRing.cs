...
namespace SpaceGraphicsToolkit.Ring
{
	...
	public class SgtRing : MonoBehaviour, CwChild.IHasChildren
	{
		...
		private void UpdateMeshes(Vector3 eye)
		{
			...
			{
				UpdateMesh(ref foregroundMesh, (float)radiusMin, (float)radiusMax, (bearing - 180.0f) * Mathf.Deg2Rad, (bearing + 180.0f) * Mathf.Deg2Rad, SEGMENT_COUNT, view, 0.0f);
		...
		private void UpdateMesh(ref Mesh mesh, float radiusMin, float radiusMax, float angleMin, float angleMax, int segments, Vector3 view, float distance)
		{
			...
			if (split == true)
			{
				...
				b.Expand(Vector3.one * radiusOuter * 2);
...
namespace SpaceGraphicsToolkit.Ring
{
	...
	public class SgtRing_Editor : CwEditor
	{
		...
		[MenuItem("GameObject/CW/Space Graphics Toolkit/Ring", false, 10)]
