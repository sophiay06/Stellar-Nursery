	...
	public class SgtJovian : MonoBehaviour, CwChild.IHasChildren
	{
		...
		private void HandleCameraPreRender(Camera camera)
		{
			if (sourceMaterial != null)
			{
				...
				if (material.IsKeywordEnabled("_SGT_FLOW") == true && flowTex != null && flowTex.dimension == UnityEngine.Rendering.TextureDimension.Cube)
				{
					material.SetTexture(_SGT_FlowTex, flowTex);
				}
		...
		public bool NeedsFlowTex
		{
			get
			{
				if (material != null && material.IsKeywordEnabled("_SGT_FLOW") == true && flowTex == null)
				{
					return true;
				}
