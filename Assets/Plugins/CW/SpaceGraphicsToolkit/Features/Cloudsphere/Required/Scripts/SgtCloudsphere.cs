...
namespace SpaceGraphicsToolkit.Cloudsphere
{
	...
	public class SgtCloudsphere : MonoBehaviour, CwChild.IHasChildren
	{
		...
		private static int _SGT_DistanceRatio       = Shader.PropertyToID("_SGT_DistanceRatio");
		...
		public bool NeedsDepthTex
		{
			get
			{
				if (sourceMaterial != null && sourceMaterial.GetTexture(_SGT_DepthTex) == null)
				{
					return true;
				}
		...
		public bool NeedsLightingTex
		{
			get
			{
				if (sourceMaterial != null && sourceMaterial.IsKeywordEnabled("_SGT_LIGHTING") == true && sourceMaterial.GetTexture(_SGT_LightingTex) == null)
				{
					return true;
				}
		...
		public bool NeedsNearTex
		{
			get
			{
				if (sourceMaterial != null && sourceMaterial.IsKeywordEnabled("_SGT_NEAR") == true && sourceMaterial.GetTexture(_SGT_NearTex) == null)
				{
					return true;
				}
		...
		private void HandleCameraPreRender(Camera camera)
		{
			...
			if (sourceMaterial != null)
			{
				...
				var absHeight     = Mathf.Abs(Vector3.Magnitude(transform.InverseTransformPoint(eye)) - radius);
				var height01      = Mathf.Clamp01(CwHelper.Divide(absHeight, radius * 0.1f));
				var distanceRatio = CwHelper.Divide(1.0f, height01);
				...
				material.SetFloat(_SGT_DistanceRatio, distanceRatio);
				...
				if (sourceMaterial.IsKeywordEnabled("_SGT_LIGHTING") == true)
				{
			...
			if (cameraOffset != 0.0f)
			{
				...
				var position  = cachedTransform.position + direction * cameraOffset;
